...
The dataset is created following a four-step process: (1) search, (2) map, (3) code, and and (4) validate. 

(1) Search: 
Extensive search of the original MIMIC-IV database is conducted to identify candidate data elements based on case-insensitive key string matching as well as consultation of the extensive online documentation both on the MIMIC website and generated by the Github community. A suite of utilty functions are created to automatically generate a listing of all the candidate MIMIC data elements based on keyword search along with relevant statistics such as min, mean, median, and max (for numeric data elements) or discrete categories and their numbers of appearance (for categorical data elements) to aid the decision making in the mapping process. < add an example? > 

(2) Map: These candidate MIMIC data elements, along with their relevant statistics, are then comprehensively documented in an [open-access spreadsheet](https://docs.google.com/spreadsheets/d/1QhybvnlIuNFw0t94JPE6ei2Ei6UgzZAbGgwjwZCTtxE/edit?usp=sharing) and mapped to their CLIF counterparts by a group of one to three expert physician scientists along with one to two data scientists. In most cases, a common set of decision labels are used to mark the mapping decision for a candidate MIMIC data element, e.g. whether it should be directly mapped as is; be further processed such as having it unit of measurement converted; or be disregarded. Examples of these common decision labels are "TO MAP, AS IS", "TO MAP, CONVERT UOM", "NO MAPPING", "UNSURE", "NOT AVAILABLE." Likewise, similar common labels are created to mark the mapping status of each candidate data element, such as its last reviewer (or the lack thereof) or the necessity of additional actions.

This open-access approach to documentation is adopted to prioritize transparency and collaboration, allowing non-code-savy clinicans easy access to evaluate and review the mapping decisions. 


(3) Code: The mapping decisions as documented in the open-access spreadsheet are coded into and executed as Python scripts using a suite of libraries including `pandas`, `duckdb`, `hamilton`, `pandera`. < Add more libraies used >

While some legacy code remains, a growing majority of the codebase follows the best ETL practices in prioritizing modularity and readability to support reproducibility and reliability. The code relies on exported copies of the mapping file as "source of truth" to avovid error-prone in-line hard-coding, such that any saved changes to the mapping file (i.e. updating the decision label of a MIMIC item from "TO MAP, AS IS" to "NO MAPPING") would be automatically integrated into the ETL output and reflected in the output.
...
cite future published papers and current working papers

### (3) Known issues or limitations
< maybe also document it in the release notes. >

#### Potential undercounting in race and ethnicity mapping
- non-hispanic
- non-unique across encounters

The race- and ethnicity-related fields in CLIF's `patient` table are sourced from a single -- and the only available -- `race` field in MIMIC-IV's `admissions` table. MIMIC's `race` field provides a very granular breakdown that is a mixture of what the medicial community has historically documented separately as "race" and "ethnicity." Some examples of the MIMIC `race` categories include "HISPANIC/LATINO - PUERTO RICAN", "SOUTH AMERICAN", "BLACK/CAPE VERDEAN", "WHITE - BRAZILIAN", and "ASIAN - ASIAN INDIAN". 

Because patients no longer answer two seperate questions but one, an Asian Hispanic patient, e.g., would now be documented as "MULTIPLE RACE/ETHNICITY" instead of "Asian" for "race" and "Hispanic" for ethnicity. This could lead to undercounting of both categories in the final mapped result. (If this patient chooses either category, e.g., "Asian", over the other, it would also lead to the undercounting of the other category, e.g. "Hispanic.") 

In rarer cases, a category like "South American" which does not fit into the contemporary U.S. racial and ethnic classification schema would also be lost in the mapping, becoming an uninformative "Other" or "Unknown" category since we cannot trace if this "South American" patient is of Spanish origin or descent (which would render them "Hispanic"). 

Each case account for only 0.1% of the patient population. 

https://github.com/MIT-LCP/mimic-code/issues/1236
