...
For the most up-to-date release, see the detailed release notes in the [CHANGELOG](https://github.com/Common-Longitudinal-ICU-data-Format/CLIF-MIMIC/blob/main/CHANGELOG.md) of this project's GitHub repository.
...
To access this version, follow the steps in the [README](http://README.md#run-the-pipeline) to check out the `release/0.2.0` branch. The newly updated tables that need to be regenerated (or generated for the first time) are: `patient`, `hospitalization`, `adt`, `ecmo_mcs`, `crrt_therapy`.
...
If you are using this release, please update all CLIF tables as they are all impacted by the time-zone update. You should also re-install or update the virtual environment following the [README](http://README.md#run-the-pipeline) since new packages were added to validate data quality.
