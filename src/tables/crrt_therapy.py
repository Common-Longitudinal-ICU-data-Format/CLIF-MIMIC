...
logger = get_logger('tables.crrt_therapy')
...
def dialysis_mapping() -> pd.DataFrame:
    logger.info("loading dialysis mapping...")
...
def extracted_crrt_events(crrt_items: pd.DataFrame, crrt_id_to_variable_mapper: dict) -> pd.DataFrame:
    logger.info("extracting crrt events...")
...
def crrt_events_pivoted_wider(extracted_crrt_events: pd.DataFrame) -> pd.DataFrame:
    logger.info("pivoting crrt events to wider...")
...
def save(crrt_events_cast_and_cleaned: pd.DataFrame) -> dict:
    ...
    logger.info("output saved to a parquet file, everything completed for the crrt therapy table!")
    return metadata

def _main():
    logger.info("starting to build clif crrt therapy table -- ")
